...
interface NotevaSDK {
  ...
  articles: {
    list(params?: { page?: number; pageSize?: number; category?: string; tag?: string; keyword?: string }): Promise<NotevaArticleListResult>;
  ...
  pages: { list(): Promise<NotevaPage[]>; get(slug: string): Promise<NotevaPage> };
  categories: { list(): Promise<NotevaCategory[]>; get(slug: string): Promise<NotevaCategory> };
  tags: { list(): Promise<NotevaTag[]>; get(slug: string): Promise<NotevaTag> };
  comments: {
    list(articleId: number): Promise<NotevaComment[]>;
    create(data: { articleId: number; content: string; parentId?: number; nickname?: string; email?: string }): Promise<NotevaComment>;
  ...
  storage: { get<T = any>(key: string, defaultValue?: T): T; set(key: string, value: any): void; remove(key: string): void; clear(): void };
  seo: {
  ...
  i18n: { getLocale(): string; setLocale(locale: string): void; addMessages(locale: string, messages: Record<string, any>): void; t(key: string, params?: Record<string, any>): string };
